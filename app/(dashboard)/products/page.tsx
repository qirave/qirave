...
export default function Products() {
	...
	if (!data.products || data.products.length === 0) {
		return (
			<NotFound>
				<Button className="h-8 gap-1" onClick={() => router.push('/products/new')}>
        ...
        <main className="grid flex-1 items-start gap-4 p-4 sm:px-6 sm:py-0 md:gap-8">
			<Tabs defaultValue="all">
				<div className="sticky flex items-center gap-2">
					<h3 className="text-primary text-lg font-semibold leading-none tracking-tight">Products</h3>
					<TabsList>
					...
					<div className="ml-auto flex items-center gap-2">
						<TooltipProvider>
							<Tooltip>
								<TooltipTrigger asChild>
									<Button onClick={() => handleViewChange('list')} size="sm" variant="outline" className={`h-7 gap-1 ${view === 'list' && 'bg-muted-active-foreground'}`}>
							...
							<Tooltip>
								<TooltipTrigger asChild>
									<Button onClick={() => handleViewChange('grid')} size="sm" variant="outline" className={`h-7 gap-1 ${view === 'grid' && 'bg-muted-active-foreground'}`}>
						...
						<Button size="sm" className="h-7 gap-1" onClick={() => router.push('/products/new')}>
							<PlusCircle className="h-3.5 w-3.5" />
							<span className="sr-only sm:not-sr-only sm:whitespace-nowrap">
								Add Product
							</span>
