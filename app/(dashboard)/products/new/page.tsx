...
export default function Page() {
    ...
    const handleCanSaleChange = () => {
        if (product.canSale) {
            ...
            setProduct((prev) => ({ ...prev, canSale: true, canPurchase: prev.canPurchase ? prev.canPurchase : false }));
        }
    };
    ...
    const handleCanPurchaseChange = () => {
        if (product.canPurchase) {
            ...
            setProduct((prev) => ({ ...prev, canPurchase: true, canSale: prev.canSale ? prev.canSale : false }));
        }
    };
	...
	return (
        <main className="w-full grid flex-1 auto-rows-max gap-4 p-4 sm:px-6 sm:py-0">
            <div className="flex items-center gap-4">
                <Button type="button" variant="outline" size="icon" className="h-7 w-7" onClick={() => router.back()} aria-label="Back">
                    <ChevronLeft className="h-4 w-4" />
                    <span className="sr-only">Back</span>
                </Button>
                <h1 className="flex-1 shrink-0 whitespace-nowrap text-xl font-semibold tracking-tight sm:grow-0">{product?.name || "New Product"}</h1>
                <Badge variant="outline" className="ml-auto sm:ml-0 capitalize">
                    {product?.status}
                </Badge>
                <div className="hidden items-center gap-2 md:ml-auto md:flex">
                    <Button type="reset" variant="outline" size="sm" onClick={handleReset}>
                        Discard
                    </Button>
            ...
            <div className="grid gap-4 md:grid-cols-[1fr_250px] lg:grid-cols-3 lg:gap-8">
                    ...
                    <Card>
                            ...
                            <div className="grid gap-6 pt-4 text-foreground">
                                <div className="flex gap-4">
                                    <div className="w-fit flex gap-2 justify-center items-center">
                                        <Checkbox id="canSale" checked={!!product.canSale} onCheckedChange={handleCanSaleChange}/>
                                        <Label 
                                            htmlFor="canSale"
                                    ...
                                    <div className="w-fit flex gap-2 justify-center items-center">
                                        <Checkbox id="canPurchase" checked={!!product.canPurchase} onCheckedChange={handleCanPurchaseChange}/>
                                        <Label 
                                            htmlFor="canPurchase"
                                            className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
                                        >
                        ...
                        <CardContent className="flex flex-col gap-4">
                                ...
                                <Table>
                                    ...
                                    <TableBody>
                                        <TableRow hidden={!product.canSale}>
                                            <TableCell><Label htmlFor="price" className="font-semibold text-right">Sales</Label></TableCell>
                                            <TableCell>
                                                <Input
                                                    id="price"
                                                    type="number"
                                            ...
                                            <TableCell>
                                                ...
                                                <TaxSelect on={["sales"]} onTaxIDChange={(id) => setProduct({ ...product, taxId: id })} />
                                            </TableCell>

                                            <TableCell>
                                                <div className="w-24">
                                                    <Select value={product.saleUnitOfMesure} onValueChange={(value: UnitType) => setProduct({ ...product, saleUnitOfMesure: value })}>
                                                        <SelectTrigger id="saleUnitOfMesure" aria-label="Mesure Unit">
                                        ...
                                        <TableRow hidden={!product.canPurchase}>
                                            <TableCell><Label htmlFor="cost" className="font-semibold text-right">Purchase</Label></TableCell>
                                            <TableCell>
                                                <Input
                                                    id="cost"
                                                    type="number"
                                                    ...
                                                    onChange={(e) => setProduct({ ...product, cost: e.target.value ? parseFloat(e.target.value) : 0 }) }
                                                />
                                            </TableCell>

                                            <TableCell>
                                                <TaxSelect on={["purchase"]} onTaxIDChange={(id) => setProduct({ ...product, taxId: id })} />
                                            </TableCell>

                                            <TableCell>
                                                <div className="w-24">
                                                    <Select value={product.purchaseUnitOfMesure} onValueChange={(value: UnitType) => setProduct({ ...product, purchaseUnitOfMesure: value })}>
                                                        <SelectTrigger id="saleUnitOfMesure" aria-label="Mesure Unit">
                                                            <SelectValue placeholder="Select Unit"/>
                                                        </SelectTrigger>
                                                        <SelectContent>
                            ...
                            <div className="grid gap-3">
                                ...
                                <Select value={product.invoicingPolicy} onValueChange={(value: InvoicePolicyType) => setProduct({ ...product, invoicingPolicy: value })}>
                                    ...
                                    {tags && tags.map((tag, i) => {
                                        return (
                                            <Badge 
                                                ...
                                                onClick={() => {
                                                    setTags(tags.filter((t) => t.name !== tag.name && t.bg !== tag.bg && t.color !== tag.color));
                                                }}
                                            >
                                                {tag.name}
                                            </Badge>
                ...
                <div className="grid auto-rows-max items-start gap-4 lg:gap-8">
                    ...
                    <Card className="border border-amber-400 bg-amber-50 bg-opacity-50">
                        ...
                        <CardContent>
                            <div></div>
                            <Button type="button" size="sm" variant="secondary" className="bg-amber-200 hover:bg-amber-600 text-amber-900 hover:text-amber-100">
                                Archive Product
                            </Button>
                        </CardContent>
                    </Card>
            ...
            <div className="flex items-center justify-center gap-2 md:hidden">
                ...
                <Button type="button" size="sm">Save Product</Button>
            </div>
        </main>
	);
}
