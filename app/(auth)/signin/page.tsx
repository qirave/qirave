	...
	return (
        <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <CardContent className="grid gap-4" role="form">
                ...
                <Button type="submit" className={`w-full ${isLoading && "bg-primary/40"}`}>
                    {isLoading && ( <LoadingSpinner className="w-5 h-5 mr-2" /> )}
                    Login
                </Button>
                <Button type="button" variant="outline" className="w-full" onClick={() => signInGoogle()}>Login with Google</Button>
