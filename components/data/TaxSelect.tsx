...
export default function TaxSelect({ on = ["sales", "purchase"], onTaxIDChange }: TaxSelectProps) {
    ...
    const TaxesSelect = useMemo(() => {
        return taxData.taxes.filter((tax) => [...tax.on].some((taxOn) => on.includes(taxOn as TaxOn & string)));
    }, [taxData, on]);
    ...
    return (
        <Select value={tsxID?.toString()} onValueChange={taxIDHandler}>
            ...
            { error ? <SelectContent>{error.message}</SelectContent> : 
                    ...
                    <SelectContent>
                        { TaxesSelect.map((tax) => (
                            <SelectItem key={tax.id} value={tax.id.toString()} className="flex px-0 py-1 items-center justify-center">
                                <Badge variant="outline" className={`w-full px-4 py-1 ${tax.on.includes("sales") ? "bg-stone-200" : "bg-teal-200"}`}>{tax.name} {tax.type === "percentage" ? `${tax.rate}%` : `${tax.rate}`}</Badge>
                            </SelectItem>
                        ))}
                    </SelectContent>
                </Suspense>
