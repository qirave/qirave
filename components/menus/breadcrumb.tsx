...
export default function QBreadcrumb() {
	...
	if (breadcrumbLinks.length > 10) {
		const dropdownPathItems = breadcrumbLinks.slice(2,breadcrumbLinks.length - 2);
							...
							<DropdownMenuContent align="start">
								{dropdownPathItems.map((item, index) => (
									<DropdownMenuItem key={index} className="capitalize">
										<Link href={`/${breadcrumbLinks.slice(0, index + 3).join("/")}`}>{item}</Link>
									</DropdownMenuItem>
								))}
							</DropdownMenuContent>
						</DropdownMenu>
						...
						<React.Fragment key={index}>
							...
							<BreadcrumbItem className="capitalize">
								{index === 1 ? (
									...
									<BreadcrumbLink asChild>
										<Link href={`/${breadcrumbLinks.slice(0, breadcrumbLinks.length - 2 + index).join("/")}`}>{item}</Link>
									</BreadcrumbLink>
								)}
							</BreadcrumbItem>
						</React.Fragment>
					...
					<React.Fragment key={index}>
						...
						<BreadcrumbItem className="capitalize">
							{index === breadcrumbLinks.length - 1 ? (
								...
								<BreadcrumbLink asChild>
									<Link href={`/${breadcrumbLinks.slice(0, index + 1).join("/")}`}>{item}</Link>
								</BreadcrumbLink>
							)}
						</BreadcrumbItem>
					</React.Fragment>
