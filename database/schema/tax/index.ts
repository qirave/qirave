...
export const taxes = pgTable('taxes', {
  ...
  on: varchar('on', {
    enum: TAXES_ON,
    length: tallestElement([...TAXES_ON]).length
  })
    .array()
    .$type<TaxOn>()
    .notNull()
    .default([...TAXES_ON]),
  calculation: varchar('calculation', {
    length: tallestElement([...TAXES_CALCULATION]).length,
    enum: TAXES_CALCULATION
  })
    .notNull()
    .$type<TaxCalculation>()
    .default('excluded'),
  ...
  type: varchar('type', {
    length: tallestElement([...TAXES_TYPES]).length,
    enum: TAXES_TYPES
  })
    .notNull()
    .$type<TaxType>()
...
export const insertTaxSchema = createInsertSchema(taxes, {
  ...
  if (data.type === 'percentage' && data.rate > 100) {
    ctx.addIssue({
      ...
      path: ['rate'],
      message:
        'Rate should be less than or equal to 100 if the type is percentage'
    });
  }
});
