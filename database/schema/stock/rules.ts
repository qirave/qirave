...
export const stockRules = pgTable('stockRules', {
  ...
  ruleType: varchar('ruleType', {
    length: tallestElement([...STOCK_RULES]).length,
    enum: STOCK_RULES
  })
    .notNull()
    .$type<StockRuleType>()
...
export const stockRulesRelations = relations(stockRules, ({ one }) => ({
  product: one(products, {
    fields: [stockRules.productId],
    references: [products.id],
    relationName: 'productStockRules'
  }),
  route: one(stockRoutes, {
    fields: [stockRules.routeId],
    references: [stockRoutes.id],
    relationName: 'routeStockRules'
  })
}));
