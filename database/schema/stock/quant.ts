...
export const stockQuantRelations = relations(stockQuant, ({ one }) => ({
    product: one(products, { fields: [stockQuant.productId], references: [products.id], relationName: 'productStockQuant' }),
    route: one(stockRoutes, { fields: [stockQuant.routeId], references: [stockRoutes.id], relationName: 'routeStockQuant' })
}));
