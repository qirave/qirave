...
import {
  STOCK_MOVES_STATUSES,
  type StockMoveStatusType
} from '@/lib/data/stockMovesStatus';
...
export const stockMoves = pgTable('stockMoves', {
  ...
  sourceRouteId: bigint('sourceRouteId', { mode: 'number' })
    .references(() => stockRoutes.id)
    .notNull(),
  destinationRouteId: bigint('destinationRouteId', { mode: 'number' })
    .references(() => stockRoutes.id)
    .notNull(),
  status: varchar('status', {
    length: tallestElement([...STOCK_MOVES_STATUSES]).length,
    enum: STOCK_MOVES_STATUSES
  })
    .notNull()
    .$type<StockMoveStatusType>()
    .default('pending'),
...
export const stockMovesRelations = relations(stockMoves, ({ one }) => ({
  product: one(products, {
    ...
    references: [products.id],
    relationName: 'productStockMoves'
  }),
  sourceRoute: one(stockRoutes, {
    fields: [stockMoves.sourceRouteId],
    references: [stockRoutes.id],
    relationName: 'sourceRouteStockMoves'
  }),
  destinationRoute: one(stockRoutes, {
    fields: [stockMoves.destinationRouteId],
    references: [stockRoutes.id],
