import { pgTable, varchar, real, text, timestamp, boolean, serial, integer } from 'drizzle-orm/pg-core';
...
export const products = pgTable('products', {
    ...
    status: varchar('status', { enum: PRODUCT_STATUSES, length: tallestElement([...PRODUCT_STATUSES]).length }).notNull().$type<ProductStatusType>().default('active'),
    supplier: varchar('supplier', { length: 255 }),
    sku: varchar('sku', { length: 50 }).unique(),
    barcode: varchar('barcode', { length: 50 }).unique(),
    tags: varchar('tags', { length: 255 }).default(''),
    invoicingPolicy: varchar('invoicingPolicy', { length: tallestElement([...PRODUCT_STATUSES]).length, enum: INVOICE_POLICIES }).notNull().$type<InvoicePolicyType>().default('ordered quantities'),
    saleUnitOfMesure: varchar('saleUnitOfMesure', { length: 50 }).notNull().default('unit'),
    purchaseUnitOfMesure: varchar('purchaseUnitOfMesure', { length: 50 }).notNull().default('unit'),

    // Direct relation to category
    categoryId: integer('categoryId').references(() => categories.id).$defaultFn(() => 2),
...
export const productsRelations = relations(products, ({ one, many }) => ({
    tax: one(taxes, { fields: [products.taxId], references: [taxes.id], relationName: 'productTaxes',  }), // Sales tax relation
    category: one(categories, { fields: [products.categoryId], references: [categories.id], relationName: 'productCategory' }), // Category relation
