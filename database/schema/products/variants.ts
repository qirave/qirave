...
export const variantValues = pgTable('variantValues', {
    id: serial('id').primaryKey(),
    variantId: bigint('variantId', {mode: 'number'}).references(() => variants.id).notNull(),
...
export const variantRestrictions = pgTable('variantRestrictions', {
    ...
    productId: bigint('productId', {mode: 'number'}).references(() => products.id).notNull(),
    variantId: bigint('variantId', {mode: 'number'}).references(() => variants.id).notNull(),
    allowedValueId: bigint('allowedValueId', {mode: 'number'}).references(() => variantValues.id).notNull(),
});
...
export const variantValueRelations = relations(variantValues, ({ one }) => ({
    variant: one(variants, { fields: [variantValues.variantId], references: [variants.id] }),
}));

// Define relations for variantRestrictions (product -> variant -> variantValue)
export const variantRestrictionRelations = relations(variantRestrictions, ({ one }) => ({
    product: one(products, { fields: [variantRestrictions.productId], references: [products.id] }),
    variant: one(variants, { fields: [variantRestrictions.variantId], references: [variants.id] }),
    allowedValue: one(variantValues, { fields: [variantRestrictions.allowedValueId], references: [variantValues.id] }),
}));
